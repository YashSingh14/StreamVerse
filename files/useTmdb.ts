...
export function useTrending(mediaType: "all" | "movie" | "tv" = "all") {
  return useQuery({
    ...
    queryFn: ({ signal }) => tmdb.getTrending(mediaType, "day", 1, signal),
...
export function useDiscover(params: Omit<DiscoverParams, "page">) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1, signal }) =>
      tmdb.discover({ ...params, page: pageParam }, signal),
    initialPageParam: 1,
...
export function useSearchMulti(query: string) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 1, signal }) =>
      tmdb.searchMulti(query, pageParam, signal),
    initialPageParam: 1,
    getNextPageParam: (lastPage) =>
      lastPage.page < lastPage.total_pages ? lastPage.page + 1 : undefined,
